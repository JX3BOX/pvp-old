<template>
	<div class="m-sandboxMap">
		...
		<div v-for="item in camps" :key="item.key">
			...
			<img class="u-icon" :src="imgPath(item.key, item.name, 'camp')" :style="positionStyle(item.name, 'icon')" style="cursor: default" />
		...
		<template v-if="maps.list && maps.list.length > 0">
			<div v-for="item in maps.list" :key="item.id">
				<img class="u-pic" :src="imgPath(item.id, item.camp, 'pic')" :style="positionStyle(item.name_pinyin, 'pic')" />
				<!-- 简介 -->
				<el-popover class="u-icon" :style="positionStyle(item.name_pinyin, 'icon')" v-for="(item, o) in maps.list" :key="o" placement="top-start" width="240" trigger="hover">
					<div class="u-box">
					...
					<div slot="reference" @click="showLog(item)">
		...
		<template v-if="maps.detail && maps.detail.length > 0">
			<div class="detail" v-for="item in maps.detail" :key="item.id">
				<img class="u-camps" :src="imgPath(item.name_pinyin, camp, 'camp')" :style="positionStyle(item.name_pinyin, 'camp')" />
				<!-- 攻防箭头 -->
				<template v-if="route">
					<div v-for="attacks in item.castles" :key="attacks.id">
						<img v-for="arr in attacks.attacks" :key="arr.id" class="u-attacks" :src="imgPath(arr, camp, 'arr')" :style="positionStyle(attacks.name_pinyin, 'arr', arr.name_pinyin)" />
					</div>
					<template v-if="item.attacks && item.attacks.length !== 0">
						<div v-for="arr in item.attacks" :key="arr.id">
							<img class="u-attacks" :src="imgPath(arr, camp, 'attacks')" :style="positionStyle(item.name_pinyin, 'attacks', arr.name_pinyin)" />
						</div>
					</template>
				</template>
			</div>
...
export default {
	...
	methods: {
		...
		imgPath(name, camp, key) {
			switch (key) {
				case "pic":
					return camp == "haoqi" ? __imgPath + "image/camp/h" + name + ".png" : __imgPath + "image/camp/e" + name + ".png";
				case "camp":
					return __imgPath + "image/camp/" + name + ".png";
				case "icon":
					return camp == "haoqi" ? __imgPath + "image/camp/h" + this.icon(name) + ".png" : __imgPath + "image/camp/e" + this.icon(name) + ".png";
				case "arr":
				case "attacks":
